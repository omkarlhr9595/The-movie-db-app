...
class TrendingBloc extends Bloc<TrendingEvent, TrendingState> {
  ...
  Future<void> _onTimeWindowChanged(TrendingTimeWindowChangedEvent event, Emitter<TrendingState> emit) async {
    final newWindow = event.timeWindow;
    ...
    try {
      ...
      final cachedOrNetwork = await _getTrendingMovies(GetTrendingMoviesParams(timeWindow: newWindow, page: 1));
      ...
      final fresh = await _getTrendingMovies(GetTrendingMoviesParams(timeWindow: newWindow, page: 1, forceRefresh: true));
      fresh.when(
        success: (List<Movie> movies) => emit(TrendingSuccessState(
          movies: movies,
          timeWindow: newWindow,
  ...
  Future<void> _onRequested(TrendingRequestedEvent event, Emitter<TrendingState> emit) async {
    ...
    try {
      final result = await _getTrendingMovies(GetTrendingMoviesParams(timeWindow: state.timeWindow, page: 1, forceRefresh: true));
      result.when(
  ...
  Future<void> _onLoadMoreRequested(TrendingLoadMoreRequestedEvent event, Emitter<TrendingState> emit) async {
    ...
    try {
      final result = await _getTrendingMovies(GetTrendingMoviesParams(timeWindow: current.timeWindow, page: nextPage));
      result.when(
