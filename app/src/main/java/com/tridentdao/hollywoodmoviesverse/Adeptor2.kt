...
class Adeptor2(private val listener: MovieItemClicked) : RecyclerView.Adapter<Card1>() {

    private val items: ArrayList<MovieList> = ArrayList()
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): Card1 {
        val view = LayoutInflater.from(parent.context).inflate(R.layout.card, parent, false)
        val viewHolder = Card1(view)
    ...
    override fun onBindViewHolder(holder: Card1, position: Int) {
        val currentItem = items[position]
        var imageDrawable: Drawable? = null
        holder.name.text = currentItem.name
        holder.progressBar.visibility = View.VISIBLE
        var link = ""
        link = currentItem.poster.toString()
        val posterlink:String
        if(link == ""){
            posterlink="https://images.unsplash.com/photo-1485846234645-a62644f84728?ixlib=rb-1.2.1&ixid=eyJhcHBfaWQiOjEyMDd9&auto=format&fit=crop&w=300&q=80"
        }else{
